...
public class FragmentContest extends BaseFragment implements FragmentContestView {
    ...
    private void addQuestions() {
        ...
        for (int i = 0; i < presenter.getQuestionsSize(); i++) {
            ...
            radioButtonsContainer.setTag("QUESTION_" + i);
            ...
            for (int j = 0; j < answers.size(); j++) {
                ...
                radioButton.setTag("ANSWER_" + j);
                radioButtonsContainer.addView(selectItem);
            }
            for (int j = 0; j < radioButtons.size(); j++) {
                radioButtons.get(j).setOnClickListener(view -> onRadioButtonClick(radioButtons, view));
            }
            container.addView(item);
        }
        container.addView(inflater.inflate(R.layout.row_fragment_questionnaire_button, container, false));
        buttonSend = container.findViewById(R.id.row_fragment_questionnaire_button);
        buttonSend.setOnClickListener(this::buttonSendClick);
    }

    private void buttonSendClick(View view) {
        HashMap<String, String> sendAnswers = new HashMap<>();
        for(int i=0; i<presenter.getQuestionsSize(); i++){
            FlexboxLayout radioButtonsContainer = container.findViewWithTag("QUESTION_" + i);
            String answer = "";
            for(int j=0; j<radioButtonsContainer.getChildCount(); j++){
                RadioButton button = radioButtonsContainer.findViewWithTag("ANSWER_" + j);
                if(button.isChecked())
                    answer = button.getText().toString();
            }
            sendAnswers.put(String.valueOf(i), answer);
        }
        presenter.sendAnswers(sendAnswers);
    }

    private void onRadioButtonClick(ArrayList<RadioButton> radioButtons, View view) {
