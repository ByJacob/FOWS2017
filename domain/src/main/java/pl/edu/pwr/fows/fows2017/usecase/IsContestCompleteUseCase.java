...
public class IsContestCompleteUseCase extends AbstractRxObservableUseCase<Boolean> {

    private ContestQuestionGateway gateway;

    public IsContestCompleteUseCase(FowsRxTransformerProvider rxTransformer, ContestQuestionGateway gateway) {
        super(rxTransformer);
    ...
    protected Observable<Boolean> createObservable() {
        return Observable.fromCallable(gateway::isComplete);
    }
}
